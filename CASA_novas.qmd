...
Regarding the methods of collection of InsideAirbnb it’s worth start mentioning that “the website only provides minimal information on the data collection process” [@Alsudais:2021]. From their [website](https://insideairbnb.com/data-assumptions/) we can see that they compile, in a quarterly basis, public information from the Airbnb website using python scripts for web-scraping. According to Alsudais, “these scripts included ones that have been “copied and pasted” from other online resources” [@Alsudais:2021]. Through this method of collection, they gather different data variables from each listing including numeric (e.g., host listings count, minimum nights, price per night), categorical (e.g., host ID), spatial (e.g.,  longitude and latitude), and textual (e.g., description) data, which is later anonymised, cleansed and aggregated in files open to the public. 
:::
...
The scraping method used by Inside Airbnb captures a complete snapshot of Airbnb listings on a quarterly basis for a given city. The dataset for London, scraped in December 2023, contains 91,778 listings, providing extensive coverage. However, some of these listings may no longer be available or were never intended for booking, serving instead as “bait and switch” listings [@Gurran:2017].  

The dataset is also constrained by the city's geographical boundary. For example, properties outside the Greater London Authority boundary may still be actively used by travelers visiting London, impacting housing dynamics and tourism patterns. Excluding these properties can underestimate Airbnb's overall impact on the local rental market and urban economy.  

**Data Accuracy**

Inside Airbnb states that it is [not responsible for the accuracy of its data](https://insideairbnb.com/data-assumptions/), as the scraped information accuracy relies on Airbnb and host-generated content. One of the issues is that Airbnb anonymizes location data, causing the scraped locations to appear scattered around the actual addresses. This creates challenges for local authorities attempting to regulate unregistered hosts in London [@ferreri_platform_2018] and introduces errors in studies analyzing Airbnb’s impact on specific neighborhoods.  

Also, hosts may misrepresent listing characteristics, such as misclassifying room types as *Entire Home* or *Private Room* (https://community.withairbnb.com/t5/Ask-about-your-listing/Entire-home-or-private-room-ANY-THOUGHTS/m-p/421087). This can lead to incorrect conclusions about the distribution of Airbnb listings across property types and their overall impact on housing availability and market dynamics.
...
As of December 2023, Inside Airbnb reports approximately 66,000 active listings in London, with 43,000 (64.01%) categorized as Entire home/apartment. Most boroughs have a higher proportion of Entire home/apartment listings compared to other room types (Fig 1), with Westminster leading at over 5,000 listings, followed by Kensington and Chelsea, and Camden.
...
These entire-home listings in London are highly commercialized, with 65.23% available for over 90 days annually and a median availability of 167 days (Fig 2). Some hosts manage an exceptionally high number of properties, with a maximum of 2,475 listings, indicating commercial operations rather than traditional hosting. Among entire-home hosts with 0 to 20 listings (Fig 3), only 40% manage a single property.
...
#Add annotation for percentage of data beyond 90 days (split into two lines)
...
#Explanation: We decided to also do figure 2 in Bokeh as we consider it would be useful for an html rendering of the file.
...
#fig3 in bokeh, for html rendering 
...
#Create a colormap based on the 'mean_rent' field
...
# Assign colors to each polygon based on the 'cluster_ranked' value
...
# Map cluster ranks to descriptions
...
# Add a manual legend to describe clusters with saturation labels
legend_patches = [
...
#Align the label of the color bar
...
diagram = Digraph(
    ...
    graph_attr={
        ...
        "splines": "polyline",    # Orthogonal lines (L-shaped)
        "nodesep": "0.6",      # Horizontal spacing between nodes
        "ranksep": "0.70",     # Vertical spacing between ranks
    },
    node_attr={
        "shape": "box",
        "fontsize": "12",
...
#Add a title node
diagram.node(
    "Title", "Figure 3: Tree Chart of Proposed Regulation                          ", 
    fontsize="13", fontcolor="black", shape="plaintext"
)

#Add nodes and edges
diagram.node(
    "A", "LONDON'S AIRBNB LISTINGS", fontsize="14", penwidth="3.0"
)
diagram.node(
    "B",
...
diagram.node(
    "F", "Hosts with > 2 listings", fontsize="12", penwidth="3.0"
)
...
diagram.node(
    "I",
    "Non-saturated boroughs",
    fontsize="12",
    penwidth="3.0",
)
diagram.node(
    "J", "up to £7,500 tax-free income", fontsize="8", fontcolor="grey", color="grey"
)
diagram.node(
    "K",
...
diagram.node("L", "Not allowed", fontsize="9", fontcolor="grey", color="grey")

diagram.node(
    "M",
    "Permit and business taxes like hotels apply \n + \n A revenue cap per each property is required, \nequivalent to the boroughs' average long-term rent",
    fontsize="9",
    fontcolor="grey",
    color="grey",
)
...
One of the main limitations in our data analysis is the data accuracy of the Inside Airbnb dataset, especially if hosts intentionally avoid platform rules. 
This, at the same time, highlights the challenges of regulatory enforcement and legislation processes (Ferreri & Sanyal 2018), limiting the effectiveness of proposed regulations. 

In conclusion, the InsideAirbnb dataset offers us a deep insight into the dynamics of short-term rentals in London, which helped us propose detailed and more targeted policy recommendations.  
By analysing the data, we can distinguish between small-scale hosts and commercial operators, proposing regulations that address the unregulated commercial short-term lets issue while protecting responsible users. However, challenges and limitations, such as data accuracy and enforcement, that highlight the need for a more complex approach to ensure compliance still remain. 
Ultimately, our tailored regulations can contribute to alleviating the housing crisis by returning properties to the long-term market.
