...
#| output: asis
from sklearn.cluster import KMeans
from bokeh.models import GeoJSONDataSource, ColorBar, LinearColorMapper, HoverTool, FixedTicker
...
geojson = boros.to_json()
geo_source = GeoJSONDataSource(geojson=geojson)

# Step 3: Create a color mapper for clusters
palette = Set2[optimal_clusters]  # Use Set2 palette
color_mapper = LinearColorMapper(palette=palette, low=0, high=optimal_clusters - 1)

# Step 4: Create Bokeh figure
p = figure(
    title="Map2 Saturation of Entire Home/apt Over 90 Days by London Borough (2023)",
    width=800,
    height=600,
    toolbar_location="right",
...
# Add polygon patches for boroughs
p.patches(
    'xs', 'ys',
    source=geo_source,
    fill_color={'field': 'cluster', 'transform': color_mapper},  # Cluster-based coloring
    line_color="black",
    line_width=0.5,
    fill_alpha=0.7,
)
...
hover.tooltips = [
    ("Borough", "@NAME"),
    ("Cluster", "@cluster"),
    ("Entire/1000 HH", "@entire_1000_household"),
]
p.add_tools(hover)

# Step 6: Add a color bar with fixed integer ticks
color_bar = ColorBar(
    color_mapper=color_mapper,
    ticker=FixedTicker(ticks=[0, 1, 2]),  
    label_standoff=8,
    width=20,
    height=300,
    location=(0, 0),
...
# Step 7: Display the dynamic map
output_notebook()  # For Jupyter Notebook visualization
show(p)
```
